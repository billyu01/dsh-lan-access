/**
 * dsh-lan-access — Browser half (built client bundle).
 *
 * This is the browser peer of `src/client.js`, compiled by hand into the
 * DSH client-bundle contract: a classic script that registers a lazy CJS
 * factory via `window.__ModuleLoader__.load({ id, factory })`. The factory
 * returns `module.exports` carrying the cordis plugin surface (`inject` +
 * `apply`), exactly like the shipped `dsh.client` packages.
 *
 * It renders a "手机访问 / Mobile Access" settings section. Each local IPv4
